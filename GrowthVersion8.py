    ...
    for ticker in SeriestickerList:
        try:
            ...
            if len(ST['stockPrice']) >= 5 :
                ...
                if RevCAGR >= 20 and RevYoY > 0 and DeltaEBITDAexRevCAGR > 0 and EBITDAexYoY > 0 and PL['ebitda'].iloc[0] < 0 and PL['ebitda'].iloc[1] < 0 and PL['ebitda'].iloc[4] < 0:
                    GV8list.append(ticker)
        except:
            continue
    dfGV8 = pd.Series(GV8list)
    dfGV8.to_csv('Screened_'+filename)

GrowthV8(CSF-File, API_key) # <- add CSV-File and API_key here
