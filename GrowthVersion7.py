    ...
    for ticker in SeriestickerList:
        try:
            ...
            if len(ST['stockPrice']) >= 5 :
                ...
                if RevCAGR >= 50 and RevYoY > 0 and DeltaEBITDAexRevCAGR > 0 and EBITDAexYoY > 0 and PL['ebitda'].iloc[0] < 0 and PL['ebitda'].iloc[1] < 0 and PL['ebitda'].iloc[4] < 0:
                    GV7list.append(ticker)
        except:
            continue
            
    dfGV7 = pd.Series(GV7list)
    dfGV7.to_csv('Screened_'+filename)
        
GrowthV7(CSV-File, API_key) # <- add CSV-File and API_key here
