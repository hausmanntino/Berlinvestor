    ...
    for ticker in SeriestickerList:
        try:
            ...
            if len(ST['stockPrice']) >= 5:
                ...
                if RevCAGR >= 20 and RevYoY > 0 and PL['ebitda'].iloc[0] > 0 and PL['ebitda'].iloc[1] > 0 and PL['ebitda'].iloc[4] > 0 and EBITDACAGR > 0 and EBITDAYoY > 0:
                    GV3list.append(ticker)
        except:
            continue
    dfGV3 = pd.Series(GV3list)
    dfGV3.to_csv('Screened_'+filename)

GrowthV3(CSV-File, API_key) # <- add CSV-File and API_key here
