    ...
    for ticker in SeriestickerList:
        try:
            ...
            if len(ST['stockPrice']) >= 5 :
                ...
                if RevCAGR >= 50 and RevYoY > 0 and PL['ebitda'].iloc[0] > 0 and PL['ebitda'].iloc[1] > 0 and PL['ebitda'].iloc[4] > 0 and EBITDACAGR >= 10 and EBITDAYoY > 0:
                    GV2list.append(ticker)
        except:
            continue
    dfGV2 = pd.Series(GV2list)
    dfGV2.to_csv('Screened_'+filename)

GrowthV2('US_ticker.csv')
